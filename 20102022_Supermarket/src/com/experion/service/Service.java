...
public class Service {
	...
	public static Product createProduct(String productType) throws ParseException {
		...
		if (productType.compareTo("Fabrics") == 0) {
			...
			double productServiceCharge = scanner.nextInt();
			int serviceCharge=(int) productServiceCharge;
			scanner.nextLine();
			...
			product = new Electronics(productCode, productName, productPrice, productServiceCharge,serviceCharge, warrantyDate);
		}
		...
		for (Customer customer : customerList) {
			if (customer.getCustomerCode().compareTo(customerCode) == 0) {
				...
				if (product instanceof Fabrics) {
					...
					System.out.println("1. Repair	2. Exchange	3. Warrenty Extend");
					...
					case 3:
						if (electronics.getWarrantyDate().after(currentDate)
								|| electronics.getWarrantyDate().equals(currentDate)) {
							electronics.warrantyExtend();
							break;
						} else
							System.out.println("Not valid for Warrenty Extend");
						System.out.println("Your service charge is ");
						break;
					}
				}
			}
